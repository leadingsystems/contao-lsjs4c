...
class binderController {
	const c_str_pathToCore = "/assets/lsjs/core";

	const c_str_pathMain = __DIR__."/../../../../../..";
	
	const c_str_pathToModules = 'modules';
	const c_str_pathToModels = 'models';
	const c_str_pathToTemplates = 'templates';

	const c_str_pathToCache = '/assets/lsjs/cache';

    const c_str_pathToJsCore = '/assets/lsjs/core';
	...
	public function __construct(
        ...
        $bln_includeApp = true,
        $str_pathToApp = "",
        $str_pathToAppCustomization = "",
        $str_pathToCoreCustomization = "",
        $whitelist = "",
        $blacklist = ""
    ) {

        $this->bln_includeCore = $bln_includeCore;
        $this->bln_includeCoreModules = $bln_includeCoreModules;
        ...
        if($blacklist) {
            $this->setModuleBlacklist($blacklist);
        }
        if($whitelist) {
	...
	protected function generateCacheHash(){
        $this->str_cacheHash = md5(""
            ...
            .($this->bln_includeCore? 1 : 0)
            .($this->bln_includeCoreModules? 1 : 0)
            .($this->bln_includeAppModules? 1 : 0)
            .($this->bln_includeApp? 1 : 0)
            .$this->str_pathToApp
            .$this->str_pathToAppCustomization
            .$this->str_pathToCoreCustomization
            .$this->str_useBlackOrWhitelist
            .json_encode($this->arr_moduleBlackOrWhitelist)
        );
    }
	...
	protected function readAllFiles() {
		if ($this->bln_includeCore) {
			$this->arr_files['mainCoreFiles'] = array(
				'lsjs' => self::c_str_pathMain.self::c_str_pathToCore.'/'.self::c_str_lsjsFileName,
				'lsjs_templateHandler' => self::c_str_pathMain.self::c_str_pathToCore.'/'.self::c_str_lsjsTemplateHandlerFileName,
				'ls_version' => self::c_str_pathMain.self::c_str_pathToCore.'/'.self::c_str_lsVersionFileName
			);
		}

		if ($this->bln_includeCoreModules) {
			$this->arr_files['coreModuleFiles_original'] = $this->readModules(self::c_str_pathMain.self::c_str_pathToCore.'/'.self::c_str_pathToModules);
	...
	protected function combineOriginalAndCustomizationFileArrays($arr_original, $arr_customization, $str_mode = 'app') {
		...
		$str_originalPathToUse = $str_mode === 'app' ? $this->str_pathToApp : self::c_str_pathMain.self::c_str_pathToCore;
